...
// https://api.github.com/search/repositories?q=language:javascript&sort=stars&order=desc&per_page=10(api for find repositories using languages)

const ExplorePage = () => {
  const [loading, setLoading] = useState(false);
  const [repos, setRepos] = useState([]);
  const [selectedLanguage, setSelectedLanguage] = useState('');
  //function to explore repository by the language
  const exploreRepos = async (language) => {
    setLoading(true);
    setRepos([]);
    try {
		// 5000 request/hr for authenticated users 
  ...
  return (
    <div className='px-4'>
      ...
      <div className='bg-glassMorphismEffect max-w-2xl mx-auto rounded-md p-4'>
